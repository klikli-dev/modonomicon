...
/**
 * Submits the ghost blocks of a {@link PreviewSnapshot} via the 26.2
 * {@link SubmitNodeCollector} path.
 * <p>
 * 26.2 port: the 26.1 version of this class tessellated into a transient {@code BufferBuilder}
 * and drew via {@code PreviewGpuUploader} (direct {@code RenderPass} submission). On 26.2 the
 * loader hooks provide a {@code SubmitNodeCollector}, so each ghost block is submitted as custom
 * geometry with {@link RenderTypes#translucentMovingBlock()}, mirroring
 * {@code MultiblockPreviewRenderer} on this branch (see 312f841).
...
public final class PreviewBlockRenderer {
    ...
    /**
     * Submits every not-yet-built relevant position of the snapshot as a translucent ghost block.
     * Already correct blocks are skipped (the world itself shows them).
     */
    public static void render(PreviewSnapshot snapshot, Level level, SubmitNodeCollector submitNodeCollector) {
