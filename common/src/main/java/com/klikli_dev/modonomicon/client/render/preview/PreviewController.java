 ...
 * </p>
 * <p>
 * 26.2 port: the submit phase uses {@link SubmitNodeCollector} (via
 * {@code submitRenderFeatures} / {@code renderMultiblock}) instead of the 26.1 direct
 * {@code PoseStack} + GPU upload path. See {@link PreviewBlockRenderer} and
 * {@link PreviewBlockEntityRenderer} for the 26.2 submission details.
 * </p>
 */
public class PreviewController {
     ...
     *
     * @param submitNodeCollector 26.2 submission target for ghost blocks and block entities.
     */
    public void renderLevel(SubmitNodeCollector submitNodeCollector) {
        if (!this.session.hasMultiblock()) {
            return;
        }
        ...
        PreviewBlockRenderer.render(snapshot, level, submitNodeCollector);
        this.blockEntityRenderer.submit(snapshot.blockEntityStates(), submitNodeCollector);
    }

    /**
     * 26.2 hook-signature-compatible entry point. The {@code levelRenderState} is kept for
     * signature compatibility with the loader event registrations; submission travels via the
     * {@code submitNodeCollector}, mirroring {@code MultiblockPreviewRenderer}.
     */
    public void renderLevel(LevelRenderState levelRenderState, SubmitNodeCollector submitNodeCollector) {
        this.renderLevel(submitNodeCollector);
    }
