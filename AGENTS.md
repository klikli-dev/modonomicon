...
- Read `gradle.properties` to find the current minecraft version (variable name `minecraft_version`) and modloader versions (variable names `neo_version` and `fabric_api_version`) used.

## Java 
Do not use fully qualified class names in code. Always import classes and use their simple names. 

## Minecraft source lookups
- EXCLUSIVELY use the `minecraft-dev` skill for all minecraft, mod and modloader (neoforge and fabric) source code lookups, diffs, and signature checks.
