...
public class EntryListButton extends Button {
    ...
    public EntryListButton(BookSearchScreen parent, BookEntry entry, int pX, int pY, OnPress pOnPress) {
        super(pX, pY, BookContentScreen.PAGE_WIDTH, 10, new TranslatableComponent(entry.getName()), pOnPress);
    ...
    public void renderButton(PoseStack ms, int mouseX, int mouseY, float partialTicks) {
        if (this.active) {
            ...
            GuiComponent.fill(ms, this.x * 2, this.y * 2, (this.x + (int) ((float) this.width * widthFract)) * 2, (this.y + this.height) * 2, 0x22000000);
            RenderSystem.enableBlend();

            if (locked) {
                RenderSystem.setShaderColor(1F, 1F, 1F, 0.7F);
                BookContentScreen.drawLock(ms, this.parent.getParentScreen().getBook(), this.x * 2 + 2, this.y * 2 + 2);
            } else {
                this.entry.getIcon().render(ms, this.x * 2 + 2, this.y * 2 + 2);
            }
            ...
            if (locked) {
                name = new TranslatableComponent(Gui.SEARCH_ENTRY_LOCKED);
            } else {
                name = new TranslatableComponent(this.entry.getName());
            }

            //TODO: if we ever add a font style setting to the book, use it here
            Minecraft.getInstance().font.draw(ms, name, this.x + 12, this.y, this.getEntryColor());
        }
    }
